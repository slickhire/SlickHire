...
def extract_mobile_number(text):
    ...
    '''
    phoneNos = re.findall(re.compile(r'([0-9]{10,12})|(\d{3}[\s-]\d{3}[\s-]\d+)|(\d{5}[\s-]\d{5})|(\d{4}[\s-]\d{4}[\s-]\d{2})'), text)
    if phoneNos:
        primaryNo =''.join(phoneNos[0])
        if len(primaryNo) == 12 and primaryNo.startswith('91'):
            primaryNo = primaryNo.replace('91', '', 1)
        primaryNo = primaryNo.replace(' ', '')
        primaryNo = primaryNo.replace('-', '')
    return primaryNo
    '''
